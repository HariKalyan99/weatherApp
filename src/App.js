...
function App(){
  ...
  const performApi = async (text) => {
    ...
    try {
      if(text.length > 0){
        const fetchApi = await axios.get(`https://api.weatherapi.com/v1/forecast.json?key=9193231c702c4bbab12104946232812&q=${text}&days=1&aqi=yes&alerts=yes`);
  ...
  const findFlag = (name) => {
    ...
    if(countryFlag){
      setDisplayFlag(countryFlag.flag);
    }
  }
  ...
  return (
    <>
      ...
      {/* // {getApi ? <BasicCard location={getLocationData}/> : flag && <h1 style={{textAlign: "center"}}>{noMatch}</h1>} */}

      {/* // {isLoading ? <CircularProgress /> : getApi && <BasicCard location={getLocationData}/> ? flag && <h1 style={{textAlign: "center"}}>{noMatch}</h1> : null} */}
